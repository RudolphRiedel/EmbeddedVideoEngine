...
#elif defined (ARDUINO_NUCLEO_F446RE) || defined (ARDUINO_RASPBERRY_PI_PICO)
    /* do nothing, not using the Arduino SPI class */
#elif defined(ESP8266)
    /* the ESP8266 Arduino core is a bit special */
    SPI.setFrequency(16UL * 1000000UL);
#elif defined(ARDUINO_METRO_M4)
    /* the Metro M4 Arduino core seems to have a bug, when setting the */
    /* SPI clock to 13...16MHz, the resulting clock is 25MHz */
    SPI.endTransaction();
    SPI.beginTransaction(SPISettings(12UL * 1000000UL, MSBFIRST, SPI_MODE0)); /* -> 12.5 MHz */
#else
    SPI.endTransaction();
