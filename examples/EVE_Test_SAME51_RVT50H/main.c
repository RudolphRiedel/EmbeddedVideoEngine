...
void init_clock(void)
{
	...
	/* configure XOSC1 for a 16MHz crystal connected to XIN1/XOUT0 */
	...
	OSCCTRL->Dpll[0].DPLLCTRLB.reg = OSCCTRL_DPLLCTRLB_DIV(3) | OSCCTRL_DPLLCTRLB_REFCLK(OSCCTRL_DPLLCTRLB_REFCLK_XOSC0_Val); /* setup PLL to use XOSC0 input, divided by 8 */
...
void init_spi(void)
{
	PORT->Group[0].DIRSET.reg = PORT_PA01; /* PD_TFT */
	PORT->Group[0].DIRSET.reg = PORT_PA00; /* CS_TFT */
	PORT->Group[0].OUTCLR.reg = PORT_PA01; /* PD_TFT */
	PORT->Group[0].OUTSET.reg = PORT_PA00; /* CS_TFT */

	/* configure SERCOM5 MISO on PB01 */
	PORT->Group[1].WRCONFIG.reg =
		PORT_WRCONFIG_WRPINCFG |
		PORT_WRCONFIG_WRPMUX |
		PORT_WRCONFIG_PMUX(MUX_PB01D_SERCOM5_PAD3) |
		PORT_WRCONFIG_INEN |
		PORT_WRCONFIG_PINMASK(0x0002) |	/* PB01 */
	...
	SERCOM5->SPI.CTRLA.bit.ENABLE = 0; /* disable SERCOM5 -> enable config */
	while(SERCOM5->SPI.SYNCBUSY.bit.ENABLE);
	SERCOM5->SPI.CTRLA.reg = SERCOM_SPI_CTRLA_MODE(3) | SERCOM_SPI_CTRLA_DOPO(0) | SERCOM_SPI_CTRLA_DIPO(3); /* MSB first, CPOL = 0, CPHA = 0, SPI frame, master mode, PAD0 = MOSI, PAD2 = MISI, PAD1 = SCK */
